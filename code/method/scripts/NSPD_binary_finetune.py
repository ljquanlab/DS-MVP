...
def load_dataloader(data_type='Test'):
    #(2)加载索引
    if data_type == 'Train':
        train_index_path = '../data_loader/NSPD/binary/NSPD_finetune_train_index.txt'
        ...
        val_index_path = '../data_loader/NSPD/binary/NSPD_finetune_val_index.txt'
        ...
        test_index_path = '../data_loader/NSPD/binary/NSPD_finetune_test_index.txt'
...
def extract_features(data_feats_list, data_labels, model_seq, train_loader):
    ...
    with torch.no_grad():
        for i, data in enumerate(train_loader):
            ...
            # train_tmp_out_af = np.column_stack((train_tmp_out, scores, af, gene_embed))
